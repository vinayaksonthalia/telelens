...
- **The fix:** `SYSTEM STOP MERGES system.metric_log` (instant, revertable with `START MERGES`) + `metric_log` disabled in the ClickHouse config for the next restart (backup kept, revert commands written into the config's own comment block). After that the scan ran clean in **3.9 s.**
